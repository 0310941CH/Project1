...
<body>
    ...
    if (isset($_GET["sort"])) {
        ...
        $stmt->execute(['scategorie' => "fans"]);
        $data = $stmt->fetchAll();

        // search order methods
        echo '<div class="sortproducts">';
        if (count($data) != 0) {
            echo '<a href="adminfans.php?sort=nAsc"><button class ="orderbutton">ABC▲</button></a>
        <a href="adminfans.php?sort=nDesc"><button class ="orderbutton">ABC▼</button></a>
        <a href="adminfans.php?sort=pAsc"><button class ="orderbutton">€▲</button></a>
        <a href="adminfans.php?sort=pDesc"><button class ="orderbutton">€▼</button></a>';
        }
        echo '</div>';
