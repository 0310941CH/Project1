...
<body>
    ...
    if (isset($_GET["sort"])) {
        ...
        $stmt->execute(['scategorie' => "laptop"]);
        $data = $stmt->fetchAll();

        // search order options
        echo '<div class="sortproducts">';
        if (count($data) != 0) {
            echo '<a href="laptop.php?sort=nAsc"><button class ="orderbutton">ABC▲</button></a>
        <a href="laptop.php?sort=nDesc"><button class ="orderbutton">ABC▼</button></a>
        <a href="laptop.php?sort=pAsc"><button class ="orderbutton">€▲</button></a>
        <a href="laptop.php?sort=pDesc"><button class ="orderbutton">€▼</button></a>';
        }
        echo '</div>';
        ...
        foreach ($data as $product) {
            ...
            echo "<br>";
            echo "<a class='shopbutton' href='cart_add.php?id=" . $product["id"] . "&&page=laptop.php'>Add to cart</a>";
            echo "</div>";
            echo "</div>";
        }
    } else {
